...
namespace Tests
{
    ...
    public class Test1_5
    {
        ...
        {
            // Null input
            TestHelpers.AssertExceptionThrown(() => { Question1_5.Compress(null); }, typeof(ArgumentException));

            // Empty input
            TestHelpers.AssertExceptionThrown(() => { Question1_5.Compress(string.Empty); }, typeof(ArgumentException));
        }

        private void ValidateResult(string input, string expectedResult)
        {
            Assert.AreEqual(expectedResult, Question1_5.Compress(input));
        }
    }
}
